...
namespace CurrencyStore.Communication
{
    public class Message
    {
        ...
        public static Message ConvertFrom(byte[] buffer)
        {
            ...
            var command = buffer[13];// (short)(BitConverter.ToInt16(buffer, 12) >> 8);
            ...
            if (command == (short)CommandType.Beep)
            {
                return msg;
            }
            ...
            if (command == (short)CommandType.Login)
            {
                if (device.PkId == 0)
                {
            ...
            if (dataLength > 0)
            {
                ...
                if (command == (short)CommandType.Detail || command == 160)
                {
                ...
                {
                    var index = 15;
                    if (command == (short)CommandType.DownLoadBlackTable)
                    {
                        dataLength += KEY_LENGTH;
                        index -= KEY_LENGTH;
                    }
        ...
        private short WriteCommand(BinaryWriter sw)
        {
            sw.Write((byte)0xA0);
            sw.Write(Command);
            return (short)((byte)0xA0 + Command);
        }
        ...
        public byte[] GetBytes(byte flag)
        {
            ...
            //Int16 val = 0xAA + 0xBB;
            if (Command == (int)CommandType.Beep)
            {
                buff = new byte[8];
                sw = GetWriter(buff);
            ...
            else if (Command == (int)CommandType.Login)
            {
                buff = new byte[16];
                sw = GetWriter(buff);
                sw.WriteShort((short)buff.Length);
            ...
            else if (Command == (int)CommandType.Detail)
            {
                buff = new byte[9];
                sw = GetWriter(buff);
                sw.WriteShort((short)buff.Length);
            ...
            else if (Command == (int)CommandType.BlackTable)
            {
                buff = new byte[13];
                sw = GetWriter(buff);
                sw.WriteShort((short)buff.Length);
            ...
            else if (Command == (int)CommandType.DownLoadBlackTable)
            {
                buff = new byte[26];
                sw = GetWriter(buff);
                sw.WriteShort((short)buff.Length);
