...
namespace CurrencyStore.Utility.File
{
    public class FileHelper
    {
        public static string ConvertPath(string relativePath)
        {
            if (relativePath.IsNotNullOrEmpty())
            {
        ...
        {
            Directory.Delete(FileHelper.ConvertPath(directoryPath), true);
        }
        public static string GetParentDirectory(string directoryPath)
        {
            if (directoryPath.IsNotNullOrEmpty())
            {
                DirectoryInfo di = Directory.GetParent(FileHelper.ConvertPath(directoryPath));

                return di != null ? di.FullName : directoryPath;
            }
        ...
        public static DataTable GetFileList(string folderPath, params string[] searchPattern)
        {
...

            {
                ...
                if (searchPattern != null && searchPattern.Length > 0)
                {
                    foreach (string pattern in searchPattern)
                    {
                        ...
                        {
                            fileList.AddRange(new DirectoryInfo(FileHelper.ConvertPath(folderPath)).GetFiles(pattern, SearchOption.TopDirectoryOnly));
                        }
                    }
                }
                ...
                {
                    fileList.AddRange(new DirectoryInfo(FileHelper.ConvertPath(folderPath)).GetFiles());
                }
                ...
                if (fileList.Count > 0)
                {
                    ...
                    foreach (FileInfo objFI in fileList)
                    {
                        objDR = result.NewRow();
