    ...
    public class OracleDbHelper : DbHelper
    {
        ...
        public override int Exec_NonQuery(CommandType commandType, string commandText, Dictionary<string, DbParameter> parameters)
        {
            ...
            using (DbConnection objDbConnection = this.GetConnection())
            {
                try
                {
                    using (DbCommand objDbCommand = this.InitDbCommand(new OracleCommand(commandText, objDbConnection as OracleConnection), parameters))
                    {
        ...
        public override DbDataReader Exec_DataReader(CommandType commandType, string commandText, Dictionary<string, DbParameter> parameters)
        {
            ...
            try
            {
                using (DbCommand objDbCommand = this.InitDbCommand(new OracleCommand(commandText, this.GetConnection() as OracleConnection), parameters))
                {
                    objDbCommand.CommandType = commandType;
            ...
            using (DbConnection objDbConnection = this.GetConnection())
            {
                try
                {
                    using (DbCommand objDbCommand = this.InitDbCommand(new OracleCommand(commandText, objDbConnection as OracleConnection), parameters))
                    {
                        ...
                        using (DbDataAdapter objDbDataAdapter = new OracleDataAdapter(objDbCommand as OracleCommand))
                        {
