...
namespace CurrencyStore.Client
{
    public class CurrencyTerminal
    {
        ...
        public bool IsRecieveNormal
        {
            get
            {
                if (this.LastRecieveDatTime.HasValue)
                {
                    return DateTime.Now.Subtract(this.LastRecieveDatTime.Value).TotalMilliseconds < 30;
                }
        ...
        public bool IsTimeOut
        {
            get
            {
                if (this.LastRecieveDatTime.HasValue)
                {
                    return DateTime.Now.Subtract(this.LastRecieveDatTime.Value).TotalMilliseconds > this.TimeOut;
                }
        ...
        public void SendCurrency()
        {
            if (this.IsConnected)
            {
                if (this.TargetCurrencyCount > 0)
                {
                    ...
                    currency.OperateTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, Helper.CurrentRandom.Next(8, 19), DateTime.Now.Minute, DateTime.Now.Second).GetBytes(); //new byte[] { 0x0C, 0x0A, 0x1E, 0x12, 0x08, 0x15 };
