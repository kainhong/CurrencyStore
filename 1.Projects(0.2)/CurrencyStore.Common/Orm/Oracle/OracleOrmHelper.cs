...
namespace CurrencyStore.Common.Orm.Oracle
{
    public class OracleOrmHelper : OrmHelper
    {
        public OracleOrmHelper()
        {
            this.DbCharacter = ":";
        }
            ...
            if (objModel != null)
            {
                ...
                foreach (KeyValuePair<string, OrmColumnAttribute> column in objMappedModel.Columns)
                {
                    if (column.Value.IsUnique)
                    {
                        ...
                        parameter = new OracleParameter(parameterName, column.Value.OracleDataType);
